...
void CGameFrame::Initialize()
{
	...
	//プレイヤーのテクスチャ読み込み
	CGameFrame::LoadTexture(&tex, "tank.png", 140, 280, D3DCOLOR_ARGB(255, 255, 255, 255));
	mPlayer.SetTexture(tex);

	// 弾のテクスチャ読み込み
	tex = nullptr;
	CGameFrame::LoadTexture(&tex, "bullet.png", 32, 16, D3DCOLOR_ARGB(255, 255, 255, 255));
	mPlayer.SetBulletTexture(tex);

	// 敵のテクスチャ読み込み
	tex = nullptr;
	CGameFrame::LoadTexture(&tex, "tank2.png", 141, 280, D3DCOLOR_ARGB(255, 255, 255, 255));
	mEnemy.SetTexture(tex);

	// プレイヤーにオーナーをセット
	mPlayer.SetOwner(this);  // thisは自分自身のポインタ
}
...
void CGameFrame::Draw2D()
{
	...
	// プレイヤー
	mPlayer.Draw(mpSprite);

	// エネミー描画
	mEnemy.Draw(mpSprite);
