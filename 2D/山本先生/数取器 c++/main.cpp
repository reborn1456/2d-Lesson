...
void Update(void)
{
	...
	if (GetAsyncKeyState(VK_SPACE) & 0x8000)
	{
		...
		{
			cntFlg = true;
			const int a = cntMachine.GetCount();   //variable with const can only be changed here when it is first created
			//a = cntMachine.GetCount();    error will happen cause you can't change const
			cout << "Current Count: " << a << endl << endl;
			cntMachine.AddCount();
			//cntMachine.DrawCount();
		}
	}
	else
	{
