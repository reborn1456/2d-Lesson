...
D3DXMATRIX pacMat;	//playerMat is 行列
...
void Update(void)
{
	...
	if (pacY + 64 >= SCRH || pacY <= 0)
	{
		pacMoveY *= -1;
	}
	else if (pacX + 64 >= SCRW || pacX <= 0)
	{
		pacMoveX *= -1;
	}
	...
	D3DXMatrixTranslation(&pacMat, pacX, pacY, 0);
}
...
void Render2D(void)
{
	...
	lpSprite->SetTransform(&pacMat);
	lpSprite->Draw(pacTex, &rcPlayer, &D3DXVECTOR3(0, 0, 0), NULL, D3DCOLOR_ARGB(255, 255, 255, 255));
