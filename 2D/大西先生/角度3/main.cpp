...
void InitBullet(void)
{
	for (int b = 0; b < bulletNum; b++)
	{
		if (!bullet[b].bulletFlg)
		{
			bullet[b].bulletFlg = true;
			bullet[b].bulletX = playerX;
			bullet[b].bulletY = playerY;

			bulletDeg = GetPlayerTargetAngle(playerX, playerY, enemyX, enemyY);

			bullet[b].bulletMoveX = cosf(D3DXToRadian(bulletDeg)) * bulletSpeed;
			bullet[b].bulletMoveY = sinf(D3DXToRadian(bulletDeg)) * bulletSpeed;
			break;
		}
	}
}
void InitBullet2(float deg, float speed)
{
	for (int b = 0; b < bulletNum; b++)
	{
		if (!bullet[b].bulletFlg)
		{
			...
			bullet[b].bulletX = playerX;
			bullet[b].bulletY = playerY;
			bullet[b].bulletMoveX = cosf(D3DXToRadian(deg)) * speed;
			bullet[b].bulletMoveY = sinf(D3DXToRadian(deg)) * speed;
			break;
		}
	}
}
...
void UpdateBullet(void)
{
	for (int b = 0; b < bulletNum; b++)
	{
		if (bullet[b].bulletFlg) 
		{
			bullet[b].bulletX += bullet[b].bulletMoveX;
			bullet[b].bulletY += bullet[b].bulletMoveY;

			if (bullet[b].bulletY < -120 || bullet[b].bulletY > SCRH + 120 || bullet[b].bulletX < -120 || bullet[b].bulletX > SCRW + 120)
			{
				bullet[b].bulletFlg = false;
			}
			D3DXMatrixTranslation(&bullet[b].bulletMat, bullet[b].bulletX, bullet[b].bulletY, 0.0f);
		}
	}
}
...
void DrawBullet(void)
{
	for (int b = 0; b < bulletNum; b++)
	{
		...
		if (bullet[b].bulletFlg)
		{
			lpSprite->SetTransform(&bullet[b].bulletMat);
...
void Update(void)
{
	...
	if (GetAsyncKeyState(VK_SPACE) & 0x8000 && frame % 8 == 0)
	{
		InitBullet();
	}
