...
{
	LoadTexture(&playerTex, "PacMan.png", 512, 64, D3DCOLOR_XRGB(255, 255, 255 /*change the rgb here to not show "a" specific colour from the sprite*/));
...
void Update(void)
{
	...
	else
	{
		...
		playerMoveX *= 0.95;  //a different way of decelerating by ~~% off

		//0の近似値を0にする
		if (playerMoveX >= -0.3 && playerMoveX <= 0.3)
		{
			playerMoveX = 0;
		}
	}
	...
	else
	{
		...
		playerMoveY *= 0.95;  //a different way of decelerating by ~~% off

		//0の近似値を0にする
		if (playerMoveY >= -0.3 && playerMoveY <= 0.3)
		{
			playerMoveY = 0;
		}
	}
	...
	//If left botton of left mouse is clicked
	if (GetAsyncKeyState(1 /*left click on mouse*/) & 0x8000)
	{
		playerX -= 2;
	}

	if (GetAsyncKeyState(2 /*right click on mouse*/) & 0x8000)
	{
		playerX += 2;
	}

	//so that it loops back to the left side if it goes out of screen
	if (playerX > SCRW)
	{
		playerX = -64;
	}
