...
void Update(void)
{
	...
	a = cosf(D3DXToRadian(deg)) * c;
	b = sinf(D3DXToRadian(deg)) * c;

	D3DXMatrixTranslation(&playerMat, playerX, playerY, 0.0f);
	D3DXMatrixTranslation(&bulletMat, playerX + a, playerY + b, 0.0f);
...
void Render2D(void)
{
	...
	lpSprite->SetTransform(&bulletMat);
	lpSprite->Draw(bulletTex, &rcPlayer, &D3DXVECTOR3(32.0f, 32.0f, 0.0f), NULL, D3DCOLOR_ARGB(255, 255, 255, 255));
