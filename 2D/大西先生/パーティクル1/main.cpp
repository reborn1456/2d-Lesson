...
// パーティクル用変数宣言
const int           particleNum = 50;     // 粒子の個数
...
void UpdateParticle(PARTICLE *p)
{
	...
	if (p->mLifespan <= 0)
	{
		EmitParticle(p, playerX, playerY+40, RND*2-1, RND*10-1, RND*3, RND*90+10);
	}
...
void DrawParticle(PARTICLE *p)
{
	...
	lpSprite->Draw(particleTex,   //何を表示しますか
		...
		&D3DXVECTOR3(16.0f, 16.0f, 0.0f),   //中心点
		NULL,   //注視点
		D3DCOLOR_ARGB(100, 255, 255, 255));   //表示させる色
}
...
void Initialise(void)
{
	...
	LoadTexture(&particleTex, "smoke.png", 32, 32, D3DCOLOR_XRGB(0, 0, 0));
	playerX = 300.f;
	playerY = 200.f;
