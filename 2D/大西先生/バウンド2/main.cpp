...
const int pacCnt = 5;  //amount of pacMan

LPDIRECT3DTEXTURE9 pacTex;
D3DXMATRIX pacMat[pacCnt];	//playerMat is 行列

float pacX[pacCnt];
float pacY[pacCnt];
float pacMoveX[pacCnt];
float pacMoveY[pacCnt];
...
void Initialise(void)
{
	...
	{
		LoadTexture(&pacTex, "PacMan.png", 512, 64, D3DCOLOR_XRGB(255, 255, 255 /*change the rgb here to not show "a" specific colour from the sprite*/));

		int rndX = rand() % SCRW - 64;
		int rndY = rand() % SCRH - 64;

		pacX[i] = rndX;
		pacY[i] = rndY;

		int rndMoveX = rand() % 2 * 20 - 10;  //random number from -10 to 10. Rand 0 or 1, then * by 20 and - 10
		int rndMoveY = rand() % 2 * 20 - 10;  //random number from -10 to 10. Rand 0 or 1, then * by 20 and - 10
		// (rand()%2)?-5:5     this is a if sentance and if rand is 0 then -5 and if rand is 1 then 5
...
void Update(void)
{
	for (int i = 0; i < pacCnt; i++)
	{
		...
		if (pacY[i] + 64 >= SCRH || pacY[i] <= 0)
		{
			pacMoveY[i] *= -1;
		}
		else if (pacX[i] + 64 >= SCRW || pacX[i] <= 0)
		{
			pacMoveX[i] *= -1;
		}

		D3DXMatrixTranslation(&pacMat[i], pacX[i], pacY[i], 0);
	}
}
...
void Render2D(void)
{
	...
	for (int i = 0; i < pacCnt; i++)
	{
		lpSprite->SetTransform(&pacMat[i]);
		lpSprite->Draw(pacTex, &rcPlayer, &D3DXVECTOR3(0, 0, 0), NULL, D3DCOLOR_ARGB(255, rand()%256, rand() % 256, rand() % 256));
	}
//	lpSprite->Draw(playerTex, &rcPlayer, &D3DXVECTOR3(0, 0, 0), NULL, D3DCOLOR_ARGB(frame % 256, frame % 256, frame % 256, frame % 256));
...
//	lpD3DDevice->Clear(0, NULL, D3DCLEAR_TARGET | D3DCLEAR_ZBUFFER, D3DCOLOR_XRGB(frame%256, frame % 256, frame % 256), 1.0f, 0);
