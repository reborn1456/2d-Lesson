...
void InitBullet(void)
{
	for (int b = 0; b < bulletNum; b++)
	{
		if (!bullet[b].Flg)
		{
			...
			bullet[b].X = playerX;
			bullet[b].Y = playerY;
			bullet[b].bulletChaseTime = 60;

			//bullet[b].bulletDeg = GetPlayerTargetAngle(playerX, playerY, enemyX, enemyY);
			bullet[b].bulletDeg = 270;

			bullet[b].MoveX = cosf(D3DXToRadian(bullet[b].bulletDeg)) * bulletSpeed;
			bullet[b].MoveY = sinf(D3DXToRadian(bullet[b].bulletDeg)) * bulletSpeed;
			break;
		}
	}
}
...
void UpdateBullet(void)
{
	...
	for (int b = 0; b < bulletNum; b++)
	{
		if (bullet[b].Flg) 
		{
			...
			{
				//bullet[b].bulletDeg = GetPlayerTargetAngle(bullet[b].X, bullet[b].Y, enemyX, enemyY);
				//　①「弾から見た」敵のいる角度を求める
				targetDeg = GetPlayerTargetAngle(bullet[b].X, bullet[b].Y, enemyX, enemyY);
				// ② 角度差を求める
				diffDeg = targetDeg - bullet[b].bulletDeg;
				// ③ 0~359度内に補正
				if (diffDeg < 0)
				{
					diffDeg += 360;
				...
				/*if (diffDeg < 180)
				{
					bullet[b].bulletDeg += turnDeg;   //時計回り（右回り）
					if (bullet[b].bulletDeg >= 360)   // 0~359度内に補正
					{
						bullet[b].bulletDeg -= 360;
					}
				}
				else
				{
					bullet[b].bulletDeg -= turnDeg;
					if (bullet[b].bulletDeg < 0)   // 0~359度内に補正
					{
						bullet[b].bulletDeg += 360;
					}
				} */
...
void Update(void)
{
	...
	//{
		...
		//a = cosf(D3DXToRadian(bullet[b].bulletDeg)) * c;
		//b = sinf(D3DXToRadian(bullet[b].bulletDeg)) * c;
